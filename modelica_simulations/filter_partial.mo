model filter_partial
  ...
  Modelica.Fluid.Vessels.OpenTank tank_B101(
      redeclare package Medium = Medium, 
      crossArea= 0.33, 
      energyDynamics = Modelica.Fluid.Types.Dynamics.FixedInitial, 
      height= 0.45, 
      level(fixed = true, start = 0.4), 
      level_start = 0.4, massDynamics = Modelica.Fluid.Types.Dynamics.FixedInitial, 
      nPorts = 2,       
  ...
  Modelica.Fluid.Vessels.OpenTank tank_B102(
      redeclare package Medium = Medium, 
      crossArea= 0.33, 
      energyDynamics = Modelica.Fluid.Types.Dynamics.FixedInitial, 
      height= 0.45, 
      level(fixed = true, start = 0.01), 
      level_start = 0.01, 
      massDynamics = Modelica.Fluid.Types.Dynamics.FixedInitial, 
      nPorts = 2,       
  ...
  Modelica.Fluid.Pipes.StaticPipe pipe6(
      redeclare package Medium = Medium, allowFlowReversal = true, diameter= 0.015, height_ab=0, 
      length=1, m_flow_start = 1) 
      annotation(
      Placement(visible = true, transformation(origin = {30, 0}, extent = {{-10, 10}, {10, -10}}, rotation = 0)));


  Modelica.Fluid.Pipes.StaticPipe pipe7(
      redeclare package Medium = Medium, allowFlowReversal = true, diameter= 0.015, height_ab=0,
      length=1) 
      annotation(
      Placement(visible = true, transformation(origin = {-30, 0}, extent = {{-10, 10}, {10, -10}}, rotation = 0)));

 Modelica.Fluid.Pipes.StaticPipe pipe10(replaceable package Medium = Medium, allowFlowReversal = true, diameter = 0.015, height_ab = 0, length = 0.1) annotation(
...
Modelica.Blocks.Sources.RealExpression realExpression(y = filter_F101.port_a.m_flow) annotation(
 ...
 Modelica.Fluid.Valves.ValveDiscrete valveDiscrete(redeclare package Medium = Medium, allowFlowReversal = true, dp_nominal(displayUnit = "Pa") = 1000, dp_start = 0.0, m_flow_nominal = 40, m_flow_start = 1) annotation(
    Placement(visible = true, transformation(origin = {120, -76}, extent = {{-10, -10}, {10, 10}}, rotation = -90)));
 Modelica.Fluid.Valves.ValveDiscrete valveDiscrete_port_in(redeclare package Medium = Medium, allowFlowReversal = true, dp_nominal(displayUnit = "Pa") = 1000, dp_start = 0.0, m_flow_nominal = 40, m_flow_start = 1) annotation(
...
equation
  valveDiscrete_port_in.open = filling.active;
  valveDiscrete.open = emptying.active;//if 500 < time and time < 1000 then true else false;
  valve_V103.opening = if filtering.active then 1 else 0;//if 500 > time or time > 1000 then 1 else 0;
  ...
  connect(valveDiscrete_port_in.port_a, port_in) annotation(
    Line(points = {{-170, 0}, {-170, 50}}, color = {0, 127, 255}));
  connect(valveDiscrete_port_in.port_b, tank_B101.ports[2]) annotation(
    Line(points = {{-170, -20}, {-120, -20}}, color = {0, 127, 255}));
 ...
 connect(alternative.outPort, initialStep.inPort[1]) annotation(
    Line(points = {{525.4, 54}, {533.4, 54}, {533.4, -8}, {271.4, -8}, {271.4, 54}, {283.4, 54}}));
 ...
 connect(valve_leaking.port_b, sink.ports[1]) annotation(
    Line(points = {{-30, -66}, {-30, -100}}, color = {0, 127, 255}));
  annotation(
